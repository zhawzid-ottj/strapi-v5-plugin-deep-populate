...
export function getFullPopulateObject(
    ...
    const attributes = Object.entries(getModelPopulationAttributes(model)).filter(([, value]) =>
        ['relation', 'component', 'dynamiczone', 'media'].includes((value as any).type)
    )

    for (const [attrName, attrObjectIn] of attributes) {

        const attrObject = attrObjectIn as any;
        ...
        if ((attrObject as any).type === 'component') {
